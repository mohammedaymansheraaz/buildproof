...
export function Settings() {
  ...
  return (
    <div className="page page--settings">
      <div className="page-heading"><div className="eyebrow"><span className="demo-chip">workspace policy</span>Release guardrails</div><h1>Safety is part of the product surface.</h1><p>These controls model the policy layer a production BuildProof deployment should enforce.</p></div>
      <div className="settings-layout">
        ...
        <div className="settings-side"><GlassPanel className="settings-principle" tone="standard"><SlidersHorizontal size={18} /><span className="panel-kicker">Policy layer</span><h2>Rules decide. Models explain.</h2><p>Severity, authorization scope, and release gates need deterministic ownership rather than model-only judgment.</p></GlassPanel><GlassPanel className="demo-reset" tone="mist"><AlertTriangle size={18} /><div><span className="panel-kicker">Workspace data</span><strong>Refresh persisted audit records.</strong><p>This reloads your Supabase-backed workspace. It does not delete audits, touch a repository, or run external traffic.</p><button type="button" className="button button--quiet" onClick={reset}><RotateCcw size={16} /> Refresh workspace data</button>{resetMessage ? <span className="reset-message"><Check size={13} />Workspace data refreshed</span> : null}</div></GlassPanel></div>
      </div>
    </div>
  );
}
